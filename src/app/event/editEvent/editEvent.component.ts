  ...
  constructor(public auth: AuthService, public router: Router, private route: ActivatedRoute,
    public data: DataService, public notify: NotificationsService, public permissions: PermissionsService) {
      ...
      this.sub = this.route.params.subscribe(params => {
          this.id = params['id'];
          if(!!this.id) {
            this.data.getPost(this.id).then(event => {
              if(!!event) {
                this.event = event;
                this.loading = false;
        ...
        if(this.event.title && this.event.title !== '') {
          if(this.event.subtitle && this.event.subtitle !== '') {
            if(this.event.html && this.event.html !== '') {
              ...
              this.data.updatePost(this.event, this.event.id).then(result => {
                this.event = result;
                this.showPreview = false;
                this.updating = false;
                this.notify.success("Event Updated", "Post has been updated successfully", true, "success");
              })
            } else {
              //content
              this.updating = false;
